...
(function ($) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (resp.result == "message sent") {
                    ...
                    var x = document.getElementById("snackbar");
                    $("#snackbar").html("Message Not Sent");
                    $('#snackbar').css('background-color', 'rgb(247, 59, 59)');
                    x.className = "show";
                    setTimeout(function () { x.className = x.className.replace("show", ""); }, 3000);
                }
    ...
    $('#cart-form').submit(function(e) {
        setTimeout(() => { console.log("Function Executed") }, 2000);
        ...
        if (userStatechecking == null) {
            ...
            $.ajax({
                ...
                dataType: "json",
                type: "POST",
                data: JSON.stringify({ email: emailId, n_items: cartnumbers, totalCost: totalCost, products_ordered: products, country: country, name: fullname, mobile_num: mobilenumber, post_code: postcode, address_1: address1, address_2: address2, landmark: landmark, city: city, state: state }),
                success: function (response) {
                    let resp = JSON.parse(JSON.stringify(response));
